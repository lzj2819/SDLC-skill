# DevForge v1.2 优化设计文档

> **设计目标**：补齐 DevForge v1.1 在全生命周期覆盖度上的短板，提升"设计→交付物"的闭环能力，增强生产就绪支持。
>
> **设计日期**：2026-04-28
> **设计版本**：v1.0
> **作者**：Claude Code + 用户协作
...
DevForge v1.1 在需求分析和架构设计阶段表现优秀（三层 XML 体系、动态模式筛选、领域扩展），但在以下方面存在明显断层：
...
### 3.1 改动 1：devforge-architecture-design 新增输出节点
...
### 3.2 改动 2：devforge-project-scaffolding CI/CD 覆盖率增强
...
### 4.1 新增 Skill：devforge-visualization（架构可视化）

#### 触发条件
- 用户输入 `[VISUALIZE]` 或 "生成架构图"
- `devforge-architecture-design` 完成后自动建议（可选）
...
### 4.2 新增 Skill：devforge-ops-ready（生产就绪基础设施）

#### 触发条件
- 用户输入 `[OPS]` 或 "生成部署配置"
- `devforge-project-scaffolding` 完成后可用
...
| 优化项 | 形态 | 触发时机 | 依赖 |
|--------|------|----------|------|
| Bug 定位与修复建议 | `devforge-debug-assistant` skill | 测试失败 / 异常日志 | 需要已有代码和测试 |
| 代码重构建议 | 同上 skill 的子功能 | 代码审查 / 设计审查 | 需要已有代码 |
| 需求追溯矩阵 (RTM) | 扩展 `devforge-requirement-analysis` | PRD 生成时自动输出 | 无 |
| 多环境部署策略 | 扩展 `devforge-ops-ready` | 用户需要蓝绿/金丝雀发布 | 需要 P1 完成 |
...
├── 1.2 更新 devforge-architecture-design/SKILL.md（第 10、11 步）
├── 1.3 更新 devforge-project-scaffolding/SKILL.md（覆盖率 CI）
├── 1.4 更新 docs/sync-rules.md（同步规则扩展）
└── 1.5 P0 验证：跑一轮完整 DevForge 流程，确认 schema.sql + openapi.yaml + 覆盖率报告正常产出

Phase 2: P1 实施
├── 2.1 创建 devforge-visualization/SKILL.md
├── 2.2 创建 devforge-ops-ready/SKILL.md
├── 2.3 创建 devforge-ops-ready/references/（云平台模板）
...
- [ ] `devforge-architecture-design` 完成后自动产出 `schema.sql` 和 `openapi.yaml`
- [ ] CI 流水线中覆盖率检查通过（≥80%）
