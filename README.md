...
安装脚本会：
1. 自动克隆仓库到 `~/.claude/devforge-skills/`
2. 为每个 Skill 创建符号链接到 `~/.claude/skills/`
3. 验证安装是否成功
...
| 方式 | 更新操作 | 缺点 |
|:---|:---|:---|
| ~~复制文件~~ | `git pull` + 重新 `cp -r` | 繁琐、易遗漏、旧文件残留 |
| **符号链接（本方案）** | 只需 `git pull` | 一次克隆，永久更新，自动同步 |

> **项目级安装**：如果你希望仅在当前项目使用 DevForge，可将 Skill 目录复制到 `{project}/.claude/skills/`。但更新时需要对每个项目重复操作。
...
ln -s ~/.claude/devforge-skills/devforge-architecture-design ~/.claude/skills/
# ... 为其他 skill 重复上述操作

# 3. Windows 用户使用目录 junction（以管理员身份运行 PowerShell）
# New-Item -ItemType Junction -Path "$env:USERPROFILE\.claude\skills\devforge-requirement-analysis" -Target "$env:USERPROFILE\.claude\devforge-skills\devforge-requirement-analysis"
```
...
> **推荐**：个人开发者用全局路径 + 符号链接（一键安装脚本默认方案）；团队协作时建议放入项目级路径并提交到 Git，确保团队成员使用相同版本。
...
| `[MODULE_BATCH {id1},{id2}]` | `devforge-module-design` | 并行批量设计多个模块 |
| `[TEST]` | `devforge-test-execution` | 运行所有测试并生成报告 |
...
```bash
# macOS / Linux
cd ~/.claude/devforge-skills && git pull

# Windows (PowerShell)
cd $env:USERPROFILE\.claude\devforge-skills; git pull
```

由于 Skill 是通过符号链接/目录联接引用的，**`git pull` 后无需任何其他操作**，Claude Code 下次启动时自动使用最新代码。

**一键更新脚本（可选）：**

```bash
# macOS / Linux
~/.claude/devforge-skills/update.sh

# Windows (PowerShell)
~\.claude\devforge-skills\update.ps1
```
...
```bash
# macOS / Linux
~/.claude/devforge-skills/uninstall.sh

# Windows (PowerShell)
~\.claude\devforge-skills\uninstall.ps1
```
...
| Skill 行为与文档不符 | 版本不匹配 | 运行 `git log --oneline -1` 确认版本为 v1.3+ |
...
| 3a | `devforge-architecture-validation` | 架构已批准 | `VALIDATION_REPORT.md` + `VALIDATION_DELTA.md` + `health-check.sh` |
| 3b | `devforge-design-review` | 架构已批准（推荐） | `DESIGN_REVIEW.md`（问题清单，非 PASS/FAIL） |
| 3c | `devforge-security-audit` | 架构已批准（可选但推荐） | `SECURITY_AUDIT_REPORT.md`（风险等级） |
| 4 | `devforge-project-scaffolding` | 架构/验证/审查已批准 | `PROJECT_SCAFFOLD/`（工程目录 + CI/CD + 部署 + 测试框架 + `.env.template`） |
| 5 | `devforge-module-design` | 脚手架完成后 `[MODULE {module_id}]` | `module-architecture.xml` + `component-spec.xml` + 模块级 PRD + 精确代码骨架 |
...
```
Design（设计）
  ├── 阶段 1: 需求分析 —— 锁定目标与边界 + 竞品调研
  ├── 阶段 2: 架构设计 —— 动态模式评估 + 三层 XML 建模
  └── 阶段 5: 模块细化 —— 组件分解与接口契约 + 代码骨架生成

Implement（实现）
  └── 阶段 4: 项目脚手架 —— 基础设施 + CI/CD + 测试框架 + 部署拓扑

Verify（验证）
  ├── 阶段 3a: 架构验证 —— LLM 沙盘模拟 + 一致性审计
  ├── 阶段 3b: 设计审查 —— 对抗性审查（攻击者/运维/扩展者视角）
  ├── 阶段 3c: 安全审计 —— 漏洞扫描 + CVE 检查
  └── 阶段 6: 测试执行 —— 单元/集成/端到端测试 + 覆盖率报告
...
├── devforge-design.md               # Skill 链分解设计文档 v1.3
...
│   └── SKILL.md
├── devforge-architecture-validation/ # 阶段 3a: 架构验证
│   └── SKILL.md
├── devforge-design-review/          # 阶段 3b: 设计审查
│   └── SKILL.md
├── devforge-security-audit/         # 阶段 3c: 安全审计
│   └── SKILL.md
├── devforge-project-scaffolding/    # 阶段 4: 项目脚手架
│   └── SKILL.md
├── devforge-module-design/          # 阶段 5: 模块细化
│   └── SKILL.md
├── devforge-test-execution/         # 阶段 6: 测试执行 (v1.3)
│   └── SKILL.md
...
| `[SKIP_REVIEW]` | 跳过设计审查 | 验证通过后直接进入脚手架 |
| `[DESIGN_REVIEW]` | 触发设计审查 | 验证通过后启动对抗性审查 |
| `[VALIDATE]` | 重新运行架构验证 | 迭代实施后或手动触发 |
| `[TEST]` | 触发测试执行 | 运行单元/集成/端到端测试 |
| `[MODULE_BATCH {ids}]` | 批量并行设计多个模块 | 同时设计多个无循环依赖的模块 |
...
- **强制阶段（1→2→4）**：需求分析 → 架构设计 → 项目脚手架，这是最小可行路径
- **推荐阶段（3a、3b、3c）**：架构验证、设计审查、安全审计，对关键系统强烈建议。3a 和 3b 默认都运行，3c 可选但推荐
- **按需阶段（5-10）**：模块细化（严格在脚手架后）、测试执行、迭代规划、可视化、运维、调试，按需触发

实际使用中，一个简单的 CRUD 应用可能 30 分钟完成（阶段 1→2→4），一个复杂微服务系统可能需要数小时（完整 10 阶段 + 多轮迭代）。
